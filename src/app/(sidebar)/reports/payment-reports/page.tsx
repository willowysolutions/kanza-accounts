...
export default async function PaymentHistoryPage({
  ...
  const paymentsByBranch = visibleBranches.map((branch: { id: string; name: string }) => ({
    ...
    customerPayments: (paymentHistory || []).filter((p: any) => p.customerId && (p.branchId === branch.id || p.customer?.branchId === branch.id)),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    supplierPayments: (paymentHistory || []).filter((p: any) => p.supplierId && (p.branchId === branch.id || p.supplier?.branchId === branch.id))
  }));
